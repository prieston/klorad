/**
 * Symmetric encryption for at-rest secrets (BYOK API keys etc.).
 *
 * Platform-shared package: used by every Klorad vertical (Campus,
 * Mobility, etc.) to store sensitive per-tenant strings encrypted in
 * Postgres. Pure utility, no DB, no env validation beyond reading
 * SECRETS_KEY at call time.
 *
