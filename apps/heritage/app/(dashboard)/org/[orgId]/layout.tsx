...
/**
 * Per-org access gate. Heritage is enabled per-organisation by an admin (the
 * Klorad admin app updates `Organization.apps`). When access is granted the
 * page is wrapped in the `DashboardShell` sidebar. When access is denied — or
 * the user is not a member of the org — we render a **bare panel** (no shell,
 * no sidebar) so the user has a clean recovery surface with Contact admin +
 * Log out, rather than nav rails to dead routes.
 */
      ...
      <div className="flex min-h-[100dvh] flex-col items-center justify-center bg-bg px-6 py-12 text-center">
        <Panel className="w-full max-w-[560px] rounded-2xl p-8">
          ...
          <h1 className="mt-4 text-lg font-semibold text-text-primary">
            Klorad Heritage is not enabled for this organisation
          </h1>
          <p className="mx-auto mt-2 max-w-md text-sm text-text-secondary">
            {member
              ? `"${member.organization.name}" doesn't have access to the Heritage app. Ask your Klorad admin to enable it, or sign out and switch accounts.`
              : "You are not a member of this organisation, or it doesn't exist."}
          </p>
          <div className="mt-6 flex flex-wrap items-center justify-center gap-2">
            <a
              href="mailto:support@klorad.com?subject=Enable%20Klorad%20Heritage"
